...
class CAVE_Server:
    ...
    def add_client (self, client):
        self.CAVE_Clients.append(client)
    def send_msg (self, *args):
        msg = pythonosc.osc_message_builder.OscMessageBuilder(address='/Visor')
        for arg in args:
            msg.add(arg)
        msg = msg.build()
        for client in self.CAVE_Clients:
            client.udp.send(msg)
...
class CAVE_Client:
    def __init__ (self, name_, ip_, port_):
        ...
        self.ip = ip_
        self.port = port_
        self.udp = pythonosc.udp_client.UDPClient(ip_, port_)
...
server = CAVE_Server()    
server.add_client(CAVE_Client('Riker', '129.161.12.147', '12345'))



, Client('Worf', '129.161.12.104'), Client('Dan', '129.161.12.147'), Client('Rouhani_Laptop', '129.161.40.113'), Client('Rouhani_Phone', '129.161.41.202')]
...
class World(DirectObject.DirectObject):
    ...
    def update (self,t):
       ...
       # print vp, vr, wp, wr
        ...
        self.display1.node().setText("visor:\n%0.3f\n%0.3f\n%0.3f\nwand:\n%0.3f\n%0.3f\n%0.3f  \nstaff:\n%0.3f\n%0.3f\n%0.3f " % (vp.getX(), vp.getY(), vp.getZ(), wp.getX(), wp.getY(), wp.getZ(), sp.getX(), sp.getY(), sp.getZ()))
